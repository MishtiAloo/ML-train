...
|---|---|---|---|
| **RetinaFace** (detector + 5-point landmarks) | `models/buffalo_l/det_10g.onnx` | Preprocessing (`01_preprocess.py`); every experiment's inference path (E1-E3, all demo modes) | Purpose-built face detector that also outputs the 5 landmarks alignment needs, so every downstream model sees a consistently cropped face instead of a raw photo. |
| **ArcFace backbone, frozen** (iResNet50) | `models/buffalo_l/w600k_r50.onnx` | Embedding cache (`03_embed.py`); **E1/E2** training (feature extractor) + inference | Already pretrained on large-scale face data, so it gives a strong 512-d identity representation for 30 people without training a backbone from scratch on a few hundred images each. |
...
- **A small CNN trained from scratch**, originally planned as a baseline to show the benefit of pretraining. Dropped: with only 30 classes and a few hundred images per identity, a from-scratch CNN was judged inherently weaker than adapting a large-scale pretrained backbone, and unlikely to add an informative data point — effort went to the E1-E3 ablation ladder instead. See `docs/final_report.tex`, Section 9 and the deviations table.
