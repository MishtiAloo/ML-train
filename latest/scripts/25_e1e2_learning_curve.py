"""Same trainer as 19_e1e2_person_finetune.py, with one addition: also
computes and logs a genuine validation loss per epoch (cross-entropy over
the full 30-person gallery, scale=args.scale, no margin, no label
smoothing -- mirrors the eval-time forward() exactly), for a real
train-loss/val-loss learning curve. 19's score() only ever tracked val
ACCURACY (the model-selection metric, since val/test people aren't part of
the 20-class locked training head -- see 19's docstring), never a loss;
this adds that missing number without touching anything else. Same seed,
same UNFREEZE_PREFIXES, same everything else, so best_epoch/val_acc/test
results are bit-for-bit identical to 19's -- only run to get the val_loss
curve, writing to a separate --out-dir so it never overwrites 19's results.

Usage (same as 19, add --fold N for the 6-fold setup file):
    CUDA_VISIBLE_DEVICES=0 python 25_e1e2_learning_curve.py --exp e1 --accept-threshold 0.3 \\
        --setup metadata/e1e2_person_folds.json --fold 1 --out-dir runs/e1e2/learning_curve
"""
...
def main():
    ...
    def score(X, y):
        """Same as 19's score(), plus val_loss: cross-entropy over the SAME
        cos matrix already computed here (zero extra forward passes),
        scale=args.scale, no margin, no label smoothing -- the natural
        loss analogue of the eval-time (labels=None) forward() pass, over
        the full 30-person gallery (not just the 20 training classes,
        since val/test people are open-set relative to the training head)."""
        model.eval()
        outs = []
        with torch.no_grad():
    ...
    print(f"  epoch   0 (untrained)  val_acc={v0['acc']*100:.2f}%  val_gap={v0['gap']:.4f}  val_loss={v0['loss']:.4f}", flush=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"  epoch {epoch:3d}  m={m:.3f}  loss={rec['train_loss']:.4f}  "
              ...
              f"val_gap={v['gap']:.4f}  val_loss={v['loss']:.4f}  ({time.time()-t0:.0f}s)", flush=True)
