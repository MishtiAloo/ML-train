...
Group No. & Not reported & Dataset version & Final six-fold study\\
Students (rolls) & \multicolumn{3}{l}{2107031, 2107044, 2107046, 2107047, 2107057, 2107059}\\
Submission date & 08/09/2026 (original) & Repository & \datasetlink\\\bottomrule
\end{tabularx}
\end{datatable}
...
Group and students & Six students: rolls 2107031, 2107044, 2107046, 2107047, 2107057, 2107059. Group number and student names are not reported in the supplied sources.\\
Affiliation & Department of Computer Science and Engineering, KUET, Khulna-9203, Bangladesh\\
Corresponding student & Name and institutional email not reported\\
...
Dataset version & Final technical-report snapshot; formal release tag not reported\\
Repository / persistent ID & \datasetlink; DOI not assigned in the supplied record\\
...
Instrument / software & Phone cameras (models unreported); InsightFace \code{buffalo\_l}; RetinaFace \code{det\_10g.onnx}; ArcFace \code{w600k\_r50.onnx}; PyTorch, ONNX Runtime and onnx2torch (versions unreported)\\
Data source location & Indoor capture; KUET-affiliated project, Bangladesh. Exact capture sites not reported.\\
Collection period & Exact start/end dates not reported; original report submitted 08/09/2026\\
...
Thirty identities (\code{p01}--\code{p30}) were enrolled. Each of six team members recruited and photographed five people: themselves and four consenting relatives or acquaintances. This is a convenience sample, not a representative demographic sample. The participant-to-photographer mapping was kept internally in \path{metadata/person_id_mapping.txt}, as described in the previous report. No demographic sampling quotas are documented.
\subsection{Collection setting, period and location}
Each participant was photographed in one sitting under regular and low indoor illumination, using a phone camera. The capture plan targeted 200 images per participant, totaling 6,000 images. Exact sites, acquisition dates, camera distances and exposure settings are not recorded in the supplied report. The original submission date was 08/09/2026. Head positions varied naturally rather than through a logged per-image pose label.
\subsection{Instruments, hardware, software and versions}
Phone camera models and capture-app versions are unreported. Preprocessing uses the project's RetinaFace detection and five-landmark alignment stage, \code{det\_10g.onnx}, within InsightFace's \code{buffalo\_l} package. Recognition uses \code{w600k\_r50.onnx} (IResNet-50). ONNX Runtime, onnx2torch and PyTorch support conversion and fine-tuning. The training record describes an RTX 3090 machine; exact package versions are not supplied.
...
Capture filenames encode identity, occlusion and lighting, for example \code{p07\_mask\_sunglass\_low\_014.jpg}. The manifest is the authoritative metadata source. Labels describe known capture conditions; a separate blind annotation study, adjudication process and inter-rater agreement are not reported.
\subsection{Provenance, versioning and raw-data preservation}
The previous report describes retaining rejected photographs in \code{put\_asides/}, with reasons, and freezing the accepted capture set before training. The final records link raw files to crops and source stems and retain hashes. A formal release identifier and change log are not supplied. Paths recorded on the training machine must be remapped when the project is moved.
...
Label quality & Capture labels and manual quality review; inter-rater agreement unreported\\
Measurement quality & Fixed alignment; 12 detector failures; camera calibration unreported\\
Integrity & Recorded hashes identify 429 duplicate occurrences; independent raw-hash audit not supplied\\
Representativeness / bias & Single-sitting convenience sample; scarf covers 24 people; demographics unreported\\
Reproducibility & Seed 42, folds, scripts and histories supplied; exact software versions missing\\\bottomrule
\end{tabularx}
\end{datatable}
...
Figure~\ref{fig:fullarcface} expands the actual \code{w600k\_r50.onnx} network, with tensor sizes verified from the supplied ONNX graph. Dimensions are $N\times C\times H\times W$ unless flattened; $N$ is the batch size. ArcFace is the angular-margin learning method, while IResNet-50 is the embedding backbone used here.
\begin{center}
...
\node[main,fill=frozen] (stem) at (0,-1.85) {\textbf{Stem:} $3\times3$ Conv, stride 1 + PReLU\\$N\times64\times112\times112$};
\node[main,fill=frozen] (s1) at (0,-3.7) {\textbf{Stage 1: 3 residual blocks}, 64 channels\\$N\times64\times56\times56$};
\node[main,fill=frozen] (s2) at (0,-5.55) {\textbf{Stage 2: 4 residual blocks}, 128 channels\\$N\times128\times28\times28$};
\node[main,fill=frozen] (s3) at (0,-7.4) {\textbf{Stage 3: 14 residual blocks}, 256 channels\\$N\times256\times14\times14$};
\node[main,fill=train] (s4) at (0,-9.25) {\textbf{Stage 4: 3 residual blocks}, 512 channels\\$N\times512\times7\times7$; last block = layer4.2};
\node[main,fill=train] (flat) at (0,-11.1) {\textbf{Embedding head: BN2 $\to$ Flatten}\\$512\times7\times7=25{,}088$ features per face};
\node[main,fill=train] (fc) at (0,-12.95) {\textbf{Fully connected projection}\\$N\times25{,}088\to N\times512$};
\node[main,fill=out] (outp) at (0,-14.8) {\textbf{Final BN1d $\to$ output embedding}\\512 values per face; $[1,512]$ for one image};
...
The project's RetinaFace stage~\cite{retinaface}, \code{det\_10g.onnx}, is always frozen: 4,225,835 parameters and 158 ONNX nodes. Figure~\ref{fig:detarchitecture} shows its three-scale outputs; Table~\ref{tab:detector} gives the tensor dimensions at $640\times640$ input.
\begin{center}
\begin{tikzpicture}
...
\node[box,fill=out,minimum height=22mm] (h3) at (0,-6.2) {12,800 face scores\\12,800 boxes\\12,800 landmark sets};
\node[box,fill=out,minimum height=22mm] (h4) at (5.75,-6.2) {3,200 face scores\\3,200 boxes\\3,200 landmark sets};
\node[box,fill=out,minimum height=22mm] (h5) at (11.5,-6.2) {800 face scores\\800 boxes\\800 landmark sets};
\draw[arrow](a)--(b);
...
\end{center}
\begin{datatable}{RetinaFace output tensors: two anchors per location; batch dimension omitted.}{tab:detector}
\begin{tabularx}{\linewidth}{Y r r r r r}\toprule
Level & Stride & Feature map & Scores & Boxes & Landmarks\\\midrule
P3 & 8 & $80\times80$ & $12800\times1$ & $12800\times4$ & $12800\times10$\\
P4 & 16 & $40\times40$ & $3200\times1$ & $3200\times4$ & $3200\times10$\\
P5 & 32 & $20\times20$ & $800\times1$ & $800\times4$ & $800\times10$\\\bottomrule
\end{tabularx}
\end{datatable}

Each level has two anchors per spatial location. The box head emits four values per anchor; the landmark head emits ten coordinates representing five points. These landmarks drive the similarity transform before the recognizer receives a crop. The detector remains fixed across E0, E1 and E2, so the adaptation comparison changes recognition rather than detection.
...
Use the files in Table~\ref{tab:files}, then run the following sequence from \code{scripts/}. Exact environment versions and machine-specific paths must be resolved before retraining.
...
\item \path{25_e1e2_learning_curve.py}: repeat E1/E2 with validation loss added.
...
\end{enumerate}
\textbf{Loss comparability.} Training uses a 20-person head, angular margin and smoothing; validation loss uses the 30-person gallery without margin or smoothing. Their absolute levels are not a directly comparable generalization gap. Section 10.7 overlays both to show epoch trends, with solid training and dashed validation lines.
...
\fig{e1_loss.pdf}{1}{E1: training loss (solid) and validation gallery loss (dashed), all six folds on one set of axes. Most folds recover from the early perturbation; the selected epoch is 5 in five folds and 8 in fold 4.}{fig:e1loss}
\fig{e2_loss.pdf}{1}{E2: both losses on the same axes, with colour identifying the fold. Greater variability and persistent validation loss accompany epoch-0 selection in three folds. The two loss definitions differ as explained in Section 10.6.}{fig:e2loss}
...
The 30-person convenience sample and single-sitting capture limit demographic, geographic and temporal generalization. Camera settings and exact acquisition periods are undocumented. Scarf appears for only 24 participants, and condition coverage is uneven. Burst correlation and 429 repeated MD5 occurrences reduce independence; image count is not an effective independent-sample count. Pose is not labeled, and regular/low lighting categories do not measure illumination quantitatively. The previous report discloses a small unflagged subset of digitally added coverings, so a strictly real-only analysis cannot be reconstructed. The 12 detector failures are absent from recognition metrics, biasing those metrics toward detectable faces. All queries belong to enrolled identities, so rejection performance for strangers is unknown. Consent records, package versions, a release tag and detailed contributor assignments are not included in the supplied report package. These limitations constrain reuse and interpretation, even when the saved experiments can be inspected.
...
Human participants / consent & Previous report states every participant signed consent before photography; participation was voluntary and withdrawable. Approval body/protocol and age/guardian details are unreported.\\
...
Web / social-media data & No such primary image source is reported.\\
...
AI-generated / synthetic content & No generative-AI collection or annotation is documented. Earlier report discloses some digitally added coverings with unknown count/provenance; final training also uses the explicit image augmentations in Section 8.\\\bottomrule
\end{tabularx}
\end{datatable}
...
Table~\ref{tab:availability} describes the available local artifacts and missing release details. This report does not establish public access to identifiable images.
...
Dataset DOI / persistent identifier & Not reported\\
Direct dataset URL & \url{https://drive.google.com/drive/u/1/folders/1XN55OGpozy5Rhbs5QRFMatlCMTAo8sPu}\\
Access instructions & Open the dataset folder; if Drive requests permission, request access from its owner. Follow participant consent conditions. Sharing permissions have not been independently tested.\\
Dataset license & Academic/non-commercial participant consent; no public release license supplied\\
Code / notebook URL & Separate code URL not supplied; local \code{scripts/} files listed in Section 10.6\\
Code version / commit / release & Commit hash and release tag not reported\\
README \& environment & Technical report and companion metrics/epoch documents supplied; exact dependency lockfile and public release README not supplied\\\bottomrule
\end{tabularx}
\end{datatable}
\section{Student Contributions (CRediT-style)}
Each team member recruited and photographed five participants (previous report, Section 4.1). Table~\ref{tab:contributions} records this shared role; names, percentage shares and individual file ownership are unreported.
\begin{datatable}{Supported student contributions; NR means not reported, not zero contribution.}{tab:contributions}
\begin{tabularx}{\linewidth}{L{23mm}L{17mm}L{24mm}Y L{32mm}}\toprule
Roll & Student & Overall share & Main contributions & Evidence / files owned\\\midrule
2107031 & NR & NR & Investigation: recruitment and capture & Prior report, Sec. 4.1\\
2107044 & NR & NR & Investigation: recruitment and capture & Prior report, Sec. 4.1\\
2107046 & NR & NR & Investigation: recruitment and capture & Prior report, Sec. 4.1\\
2107047 & NR & NR & Investigation: recruitment and capture & Prior report, Sec. 4.1\\
2107057 & NR & NR & Investigation: recruitment and capture & Prior report, Sec. 4.1\\
2107059 & NR & NR & Investigation: recruitment and capture & Prior report, Sec. 4.1\\\bottomrule
\end{tabularx}
\end{datatable}
\section{Acknowledgements, Funding and Competing Interests}
\subsection{Acknowledgements / Funding}
The report acknowledges the consenting participants and course instructors Dr. Muhammad Aminul Haque Akhand and Nabil Faiyaz Sadi. Funding information is not reported in the source documents; no funding declaration is inferred.
\subsection{Declaration of Competing Interests}
A competing-interests declaration is not provided in the supplied sources. This field requires an author declaration before any formal public release.
...
Findable & Repository URL; meaningful names; searchable metadata & Yes & Dataset folder URL supplied; meaningful filenames and CSV metadata exist. A DOI is not supplied.\\
...
Reusable & Explicit license, README, provenance, limitations and citation & No & Limitations/provenance described; public license, release README and persistent citation missing.\\
...
Versioning & Version/date and change history recorded & No & Final experiment snapshot identifiable; no formal dataset release tag or change log supplied.\\
...
Documentation & README covers files, variables, setup and reuse & No & This report and companion documents cover these topics; a complete release README/environment is absent.\\
Team accountability & All six students' contributions recorded & No & Rolls and shared collection role recorded; names, percentages and individual file ownership unreported.\\\bottomrule
\end{tabularx}
\end{datatable}
